...
#define Map(K, V) struct {                                                                                          \
        List(List(struct {K key; V value; })) table;                                                                \
        int* _p_size;                                                                                               \
...
#define _item_type(m)                       typeof(at(at_q((m).table, 0), 0))
...
#define DECLARE_MAP_TYPE(MapType, K, V, hash_func, cmp_func, new_map_func_name)                                          \
    ...
    MapType new_map_func_name(Scope* scope) {                                                                       \
        MapType m;                                                                                                  \
        m.table = (typeof(m.table)) _new_list(typeof(at_q(m.table, 0)), scope, BUCKET_SIZE);                        \
        ...
        for(int i = 0; i < BUCKET_SIZE; i++) {                                                                      \
            at_q(m.table, i) = (typeof(at_q(m.table, i))) _new_list(_item_type(m), scope, 0);                       \
        }                                                                                                           \
        return m;                                                                                                   \
    }

#define map_size(m)     (                                                                                           \
                            ((m)._state == VALID && (m).scope->state == VALID)?                                     \
                            *(m)._p_size :                                                                          \
                            QUIT(ERR_MAP_SIZE, __FILE__, __LINE__).value                                            \
                        )
...
#define map_put(m, key, value) {                                                                                    \
        ...
        int index = (unsigned) hash % _list_size(m.table);                                                          \
                                                                                                                    \
        if(_list_size(at_q(m.table, index)) == 0) {                                                                 \
            int n_bucket = _list_size((m).table);                                                                   \
            int active_bucket = ++(*(m)._p_active_bucket);                                                          \
                                                                                                                    \
            if(active_bucket > n_bucket/2) {                                                                        \
                resize_list((m).table, 3*n_bucket/2);                                                               \
            }                                                                                                       \
        }                                                                                                           \
        _item_type(m) new_item = {key, value};                                                                      \
        push(at_q(m.table, index), new_item);                                                                       \
        *(m)._p_size += 1;                                                                                          \
    }
...
#define map_get(m, _key, p_value, default_value) {                                                                                 \
        ...
        int index = (unsigned) hash % _list_size(m.table);                                                          \
        typeof(at_q(m.table, 0)) lst_items = at_q(m.table, index);                                                  \
        bool found = FALSE;                                                                                         \
        ...
        if(!found) *(p_value) = default_value;                                                                      \
    }
...
static inline  int hash_string(String st)
{
    ...
    int hash = 0;
    for(int i = 0; i < str_len(st) / 4; i++) hash |= *p++;
    return hash;
}
