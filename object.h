...
#define Object(type)                struct { type* _ptr; int _state; Scope* scope;  }

#define raw_ptr(obj)                (((obj)._state == VALID && (obj).scope->state == VALID)? (obj)._ptr: \
                                        (typeof(obj._ptr)) QUIT(ERR_RAW_PTR, __FILE__, __LINE__).ptr)

#define attr(obj, field)            (((obj)._state == VALID && (obj).scope->state == VALID)? (obj)._ptr: \
                                        (typeof(obj._ptr)) QUIT(ERR_RAW_PTR, __FILE__, __LINE__).ptr)->field

#define new_object(type, scope)     {zero_alloc(scope, sizeof(type)), VALID, scope }
#define null_object(scope)          {null, INVALID, scope }
#define make_object(scope, ptr)     {ptr, VALID, scope }
